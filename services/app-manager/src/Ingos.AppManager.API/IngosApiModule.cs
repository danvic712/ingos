...
using System.IO;
using System.Linq;
using Ingos.AppManager.API.Infrastructure;
using Ingos.AppManager.Application;
...
using Ingos.AppManager.Infrastructure;
...
namespace Ingos.AppManager.API
{
    /// <summary>
    ...
    public class IngosApiModule : AbpModule
    {
            ...
            app.UseSwaggerUI(options =>
            {
                ...
                foreach (var apiVersion in apiVersionList)
                    options.SwaggerEndpoint($"/swagger/{apiVersion}/swagger.json",
                        $"Ingos App Manager API {apiVersion?.ToUpperInvariant()}");
            });
        ...
        private void ConfigureVirtualFileSystem(ServiceConfigurationContext context)
        {
            ...
            if (hostingEnvironment.IsDevelopment())
                ...
                {
                    options.FileSets.ReplaceEmbeddedByPhysical<IngosDomainSharedModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $@"..{Path.DirectorySeparatorChar}/Ingos.AppManager.Domain.Shared"));
                    options.FileSets.ReplaceEmbeddedByPhysical<IngosDomainModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $@"..{Path.DirectorySeparatorChar}/Ingos.AppManager.Domain"));
                    options.FileSets.ReplaceEmbeddedByPhysical<IngosApplicationContractsModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $@"..{Path.DirectorySeparatorChar}/Ingos.AppManager.Application.Contracts"));
                    options.FileSets.ReplaceEmbeddedByPhysical<IngosApplicationModule>(
                        Path.Combine(hostingEnvironment.ContentRootPath,
                            $@"..{Path.DirectorySeparatorChar}/Ingos.AppManager.Application"));
                });
        }
                ...
                options =>
                {
                    ...
                    foreach (var description in provider.ApiVersionDescriptions)
                        options.SwaggerDoc(description.GroupName, new OpenApiInfo
                        {
                            ...
                            Description = "Ingos App Manager API",
                            Title = "Ingos API",
                    ...
                    {
                        @"wwwroot/api-doc/Ingos.AppManager.API.xml",
                        @"wwwroot/api-doc/Ingos.AppManager.Application.xml",
                        @"wwwroot/api-doc/Ingos.AppManager.Application.Contracts.xml"
                    };
